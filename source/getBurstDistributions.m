...
if p.Results.display
    ...
    dispfcn(num2str([meanAP,threshold,mean(stats.AP)],...
        ['Mean AS-AP: %.5g ',char(956),'V, AS amplitude threshold: %.5g ',...
        char(956),'V, Mean valid burst peak amplitude: %.5g ',char(956),'V']));
    dispfcn([num2str(expAP,['Exponential mean: %.5g ',char(956),'V']),...
        num2str(logAP,', Lognormal mean/(std): %.3g/(%.3g)'),...
        num2str(gamAP,[', Gamma (k,',char(952),'): (%.3f,%.5g)'])]);
    dispfcn(num2str([meanCN,logCN],...
        'Mean cycle number: %.2f, Lognormal mean/(std): %.3g/(%.3g)'));
    dispfcn(num2str([meanBF,logBF],...
        'Mean burst frequency: %.2f Hz, Lognormal mean/(std): %.3g/(%.3g)'));
